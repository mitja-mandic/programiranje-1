...
let initial_grid : grid = (empty_row, empty_row, empty_row)
...
let is_full_row1 (x1, x2, x3) = if (x1 = None || x2 = None || x3 = None) then false else true

(* dvojni enačaj primerja če sta objekta ista *)
...
(* sprejme triple, ki ne vemo katerega tipa je, vrne tip player*)
let winner_of_triple triple : player option = match triple with
  | (Some X, Some X, Some X) -> Some X
  | (Some O, Some O, Some O) -> Some O
  | _ -> None

let winner_of_triple1 triple : player option = match triple with
...
let rec loop state =
  ...
  | OnTurn {player; grid} -> (
    ...
    let col_index = choose "stolpec" in
