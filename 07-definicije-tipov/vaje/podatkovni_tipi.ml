...
type euro = Euro of float (* | Cent of float         Euro apliciran na tip float, cent je dodatek*)
type dollar = Dollar of float

(* let en_evro = Euro 1.0 Evro(1.0) python ekvivalent *)
...
let dollar_to_euro_alt (Dollar d) = Euro(d *. razmerje)
...
let rec intbool_separate l =
       ...
       | Empty -> (inti, booli)
       | Int(x, rep) -> separation_pomozna (x::inti) booli rep
       | Bool(x, rep) -> separation_pomozna inti (x::booli) rep
       in
       separation_pomozna [] [] (intbool_reverse l)
...
let rec find_candidate magic specialisation wizard_list = match wizard_list with
       | [] -> None
       | wizard::rest -> (
...
let rec find_candidate_alt magic specialisation wizard_list = match wizard_list with
       | [] -> None
       | wizard::rest -> (
              match wizard.status with
              | Student (student_magic, years) when student_magic = magic && zahtevana_leta specialisation <= years->
                            Some wizard.name 
              | _ -> find_candidate magic specialisation rest
       )
