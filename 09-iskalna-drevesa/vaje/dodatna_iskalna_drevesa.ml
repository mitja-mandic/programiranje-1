...
let rec mirror drevo = match drevo with
     | Prazno -> Prazno
     | Vozlisce (l, x, r) -> Vozlisce (mirror r, x, mirror l)
...
(*----------------------------------------------------------------------------*]
 ...
 - : int = 6
[*----------------------------------------------------------------------------*)
(* prostorska zahtevnost O(visina) *)

let rec size = function
     | Prazno -> 0
     | Vozlisce (l,_,r) -> 1 + size l + size r
...
let rec insert x = function
    | Prazno -> leaf x
    | Vozlisce (l, y, r) -> 
        if x < y then Vozlisce (insert x l,y,r) else
        if y < x then Vozlisce (l,y,insert x r) else
        Vozlisce(l,x,r)
...
let rec member2 x = function
    | Prazno -> false
    | Vozlisce(l,y,r) -> member2 x r || member2 x l || (x=y)
...
let rec delete x = function
    | Prazno -> Prazno
    | Vozlisce(l,y,r) -> 
        if x < y then Vozlisce (delete x l,y,r) else
        
        if x > y then Vozlisce (l,y,delete x r) 
            else (
...
let bst_of_list lst = 
    let rec aux drevo sez = match sez,drevo with
        | ([],_) -> drevo
        | (x::xs,Prazno) -> aux (leaf x) xs
        | (x::xs, drevo) -> aux (insert x drevo) xs in
    aux Prazno lst
