...
let rec sum_int_pairs = function
  | [] -> []
  | (prvi, drugi) :: ostalo -> (prvi + drugi) :: sum_int_pairs(ostalo)
...
let rec second_largest seznam =
  ...
  let rec najvecji seznam = match seznam with
    | [] -> failwith "podal si prazen seznam"
    | [x] -> x
    | x :: (y :: xs) -> if x > y then najvecji(x::xs) else najvecji(y::xs)
  in najvecji(remove (najvecji seznam)seznam)