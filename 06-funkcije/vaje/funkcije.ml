...
[*----------------------------------------------------------------------------*)
(*deluje, ampak počasi *)
let rec reverse_kvadraticen seznam = match seznam with
  | [] -> []
  | a :: xs -> (reverse_kvadraticen xs) @ [a]
...
let rec unzip = function 
  ...
  | (x,y) :: l -> 
    ...
    (*let (xs, ys) = rekurzija in  druga možnost razpakiranja *)
...
let rec unzip_tlrec l= 
  let rec unzip_pom (xs, ys) l = match l with
    | [] -> (reverse_bolje xs, reverse_bolje ys)
...
let rec loop con f x = if con x then loop con f (f x) else x 
...
let rec apply_sequence f x0 n =
  let rec apply_pomozna acc x n =
    if n < 0 then reverse_bolje acc (* reverse lahko uporabimo tut na koncu *)
...
let rec exists f = function (*function vzame zadnji argument*)
  | [] -> false
  | x::xs -> if f x then true else exists f xs
  (* | x::xs -> f x || exists f xs je tudi tail recursive, tudi za && je *) 
